...
import DateTimePicker, {DateTimePickerAndroid,
...
const RaiseConcernScreen = () => {
  ...
  const filteredData = Array.isArray(concerns.data) ? concerns.data.filter((item) => {
    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerRow}>
        <View style={styles.tabRowInline}>
          ...
          >
            <Text style={[styles.tabText, activeTab === "open" && styles.activeTabText]}>Open Concern</Text>
          </TouchableOpacity>
          ...
          >
            <Text style={[styles.tabText, activeTab === "past" && styles.activeTabText]}>Past Concern</Text>
          </TouchableOpacity>
        </View>
      ...
      {isLoading ? (
        <ActivityIndicator size="large" color="#8b1c1c" style={{ marginTop: 50 }} />
      ) : (
        <ScrollView contentContainerStyle={{ paddingBottom: 20 }}>
          {filteredData.length > 0 ? (
            filteredData.map((item) => (
              <View key={item._id} style={styles.concernCard}>
                <View style={styles.cardHeader}>
                  <Text style={styles.cardTypeText}>{item.concernType?.join(", ")}</Text>
                  <View style={[styles.statusBadge, { backgroundColor: item.resolutionStatus === 'Pending' ? '#FFF3E0' : '#E8F5E9' }]}>
                    <Text style={[styles.statusText, { color: item.resolutionStatus === 'Pending' ? '#EF6C00' : '#2E7D32' }]}>
                ...
                <View style={styles.cardFooter}>
                   <Text style={styles.dateInfo}>Meeting: {moment(item.suggestedMeetingDate).format("DD MMM, YYYY | hh:mm A")}</Text>
                </View>
              </View>
            ))
          ) : (
            <View style={styles.emptyCard}>
              <Text style={styles.cardTitle}>No {activeTab === "open" ? "Open" : "Past"} Concerns</Text>
              <Text style={styles.cardDescText}>
                You are a valuable part of our team! If you have any concerns, feel free to reach out.
              </Text>
            </View>
          )}
        </ScrollView>
          ...
          <View style={styles.modalWrapper}>
            <View style={styles.modalCard}>
                ...
                <View style={styles.formContainer}>
                  ...
                  <DropDownPicker
                    ...
                    items={[
                      ...
                      { label: "Managerial Issues", value: "Managerial Issues" },
                  ...
                  <TouchableOpacity onPress={openDatePicker}>
                    <View style={styles.dateInputWrapper}>
                      ...
                      <View style={styles.calendarIcon}>
                        <Image source={require("../assets/calenderslide.png")} style={{ width: 20, height: 20 }} />
                      </View>
                    </View>
                  </TouchableOpacity>
                </View>
              ...
              <View style={styles.modalFooter}>
                <TouchableOpacity style={styles.cancelBtn} onPress={() => setShowRaiseConcern(false)}>
...
const styles = StyleSheet.create({
  ...
  headerRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", padding: 16, marginTop: 60 },
  tabRowInline: { flexDirection: "row", backgroundColor: '#eee', borderRadius: 8, padding: 4 },
  tabBtn: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6 },
  activeTab: { backgroundColor: "#fff" },
  tabText: { fontSize: 13, color: '#666' },
  activeTabText: { color: '#8b1c1c', fontWeight: 'bold' },
  raiseBtn: { backgroundColor: "#8b1c1c", paddingHorizontal: 14, paddingVertical: 8, borderRadius: 6 },
  raiseText: { color: "#fff", fontSize: 13, fontWeight: '600' },
  
  // Concern Card Styles
  concernCard: { backgroundColor: '#fff', marginHorizontal: 16, marginTop: 12, padding: 16, borderRadius: 12, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 2 },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
  cardTypeText: { fontSize: 15, fontWeight: 'bold', color: '#333' },
  statusBadge: { paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6 },
  statusText: { fontSize: 11, fontWeight: 'bold' },
  cardDescription: { fontSize: 13, color: '#666', marginBottom: 10, lineHeight: 18 },
  cardFooter: { borderTopWidth: 1, borderTopColor: '#f0f0f0', paddingTop: 8 },
  dateInfo: { fontSize: 11, color: '#999' },

  emptyCard: { backgroundColor: "#d9cfd3", margin: 20, padding: 30, borderRadius: 12, alignItems: "center" },
  cardTitle: { fontSize: 16, fontWeight: 'bold', color: '#8b1c1c', marginBottom: 8 },
  ...
  modalCard: { backgroundColor: "#fff", borderRadius: 16, maxHeight: "90%", overflow: "hidden" },
  modalHeader: { backgroundColor: "#f1e6e9", padding: 16, flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  ...
  calendarIcon: { position: "absolute", right: 14, top: "50%", transform: [{ translateY: -5 }] },
  modalFooter: { flexDirection: "row", justifyContent: "flex-end", padding: 16, borderTopWidth: 1, borderColor: "#eee" },
  cancelBtn: { paddingHorizontal: 20, paddingVertical: 10, borderRadius: 20, borderWidth: 1, borderColor: "#8b1c1c", marginRight: 12 },
  cancelText: { color: "#8b1c1c", fontWeight: "500" },
  submitBtn: { paddingHorizontal: 22, paddingVertical: 10, borderRadius: 20, backgroundColor: "#8b1c1c" },
